 ...
 */
const getWeatherForDay = (address: string) => {
    return new Promise<string>((resolve, reject) => {
        getAMapGeocode(address).then((response: any) => {
            ...
            request(url, function (error, response, body) {
                if (response && response.body) {
                    let result1: any = convert.xml2json(response.body, { compact: true, spaces: 4 });
                    ...
                    if (lastWeekGroupBeDay['subgroup']) {
                        lastWeekGroupBeDay['subgroup'].forEach((item: any, index: number) => {
